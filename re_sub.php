...
<body style="background-image: url('img/6.jpg');">
	...
	<h4 style="text-align:center"><label> REISSUANCE DC ENTRY (ONLY To S/C) </label></h4>
	...
	<script>
		function preload(form)
		{
			...
			self.location=<?php echo"'re_sub.php?dc='"?>+p0+'&tdate='+p1+'&iss='+p;
		}
		function reload(form)
		{
			...
			self.location=<?php echo"'re_sub.php?dc='"?>+p0+'&tdate='+p1+'&sccode='+p2+'&iss='+p;
		}
		function reload0(form)
		{
			...
			self.location=<?php echo"'re_sub.php?tdate='"?>+p1+'&dc='+p0+'&sccode='+p2+'&partnumber='+p3+'&iss='+p;
		}
	</script>
			...
			<div id="stylized" class="myform"><br>
				...
				<datalist id="partlist" >
					<?php
						if(isset($_GET['sccode']))
						{
							...
							{
								$query = "SELECT distinct pn_st.invpnum as pn FROM `dcmaster` JOIN pn_st ON dcmaster.pn=pn_st.pnum where sccode='$max1'";
							}
							$result = $con->query($query);
							echo"<option value=''>Select one</option>";
							while ($row = mysqli_fetch_array($result)) 
				...
				<?php
					if(isset($_GET['partnumber']) && $_GET['iss']=="To S/C")
					{
						...
						$r="A";
						$query2 = "select prcno as rcno,sum(partreceived)-sum(partissued)-sum(qtyrejected) as qty from d12 where stkpt='To S/C' and d12.pnum='$mat' and prcno in (select DISTINCT d11.rcno from d11 join d12 on d11.rcno=d12.prcno where d12.pnum='$mat' and d11.rcno!='') group by prcno HAVING qty>0";
				...
				<script>
					function reload2(form)
					{
						...
						var p2 = document.getElementById("p2").value;
						var p3 = document.getElementById("p3").value;
						self.location=<?php echo"'re_sub.php?tdate='"?>+p1+'&dc='+p0+'&sccode='+p2+'&partnumber='+p3+'&iss='+p+'&mat='+val;
					}
				</script>
				...
				<?php				
					...
					{
						$rat = $_GET['partnumber'];
						$result = $con->query("SELECT pnum FROM `pn_st` WHERE invpnum='$rat' and stkpt='FG For Invoicing' AND n_iter='1'");
						//echo "SELECT pnum FROM `pn_st` WHERE invpnum='$rat' and stkpt='FG For Invoicing' AND n_iter='1'";
						$c = $result->num_rows;
						if($c>0)
						{
							$count=1;
				...
				<?php
					if($max>1)
					{
						echo '<br><br><div><input type="submit" name="alfaok" value="ENTER"></div>';
					}
				?>
