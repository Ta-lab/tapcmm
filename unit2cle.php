...
{
	if($_SESSION['access']=="To S/C" || $_SESSION['access']=="From S/C" || $_SESSION['access']=="Semifinished1" || $_SESSION['access']=="Semifinished2" || $_SESSION['access']=="133" || $_SESSION['access']=="NONE" || $_SESSION['access']=="ALL")
	{
...
<script>
	...
	<div style="float:right">
		...
		<h4 style="text-align:center"><label>CLE PART ISSUANCE & ROUTE CARD GENERATION</label></h4>
		<div class="divclass">
		<?php
		if(isset($_POST['submit'])){
			...
			header("Location: unit2cle_db.php?tdate=$tdate&prcno=$prcno&operation=$stkpt&ircn=$ircn&pnum=$pnum&iss=$iss&pn=$pn&type=$type ");
		}
		...
		<form method="POST" action='unit2cle.php'>	
			</br>
			<div>
				<label>DATE</label>
			...
			<div>
				<label for="operation">STOCKING POINT</label>
					<?php
						...
						//}
						//else{
							$result = $con->query("SELECT DISTINCT operation FROM m11 where opertype='OPERATION' AND operation='CLE UNIT 2'");
						//}
				...
				<script>
					...
					{	
						...
						self.location='unit2cle.php?tdate='+s+'&cat=' + val ;
					...
					{	
						...
						if(obj !=null && obj.length>0)
						{
							...
							self.location=<?php if(isset($_GET['cat']))echo"'unit2cle.php?tdate=".$_GET['tdate']."&cat=".$_GET['cat']."&rat='";?> + s ;//page name
						}
						else
						{
							myFunction();
			...
			<input type="text" list="combo-options" required name ="pnum" id="pnum" onchange="reload1(this.form)" value="<?php
				...
				if(isset($_GET['rat']) && isset($_GET['cat']) && $_GET['cat']!="Rework"  && $_GET['cat']!="From S/C")
				{
					...
					$query = "SELECT * FROM `clemaster` WHERE pnum='$pnum' ";
					$result = $con->query($query);
					$row2 = mysqli_fetch_array($result);
					if($row2['pnum'] != $pnum)
					{
						header("location: inputlink.php?msg=33");
					}
				...
				/////////////////////SF LOCK
				/*
				if(isset($_GET['rat']) && isset($_GET['cat']) && $_GET['cat']!="" && $_GET['rat']!="" && ($_GET['cat']=="Semifinished1"  || $_GET['cat']=="Semifinished2"))
				{
					$cat=$_GET['cat'];
					$rat=$_GET['rat'];
					$query = "SELECT MAX(days) c FROM (select d11.pnum as pnum,stkpt,prcno as prc,d12.date,sum(partreceived)-sum(partissued) as s ,datediff(NOW(),d12.date) as days from d12 join d11 on d12.prcno=d11.rcno GROUP BY prcno,stkpt HAVING (sum(partreceived)-sum(partissued))>0 ORDER BY d11.pnum) T where stkpt='$cat'";
					//echo "SELECT MAX(days) c FROM (select d11.pnum as pnum,stkpt,prcno as prc,d12.date,sum(partreceived)-sum(partissued) as s ,datediff(NOW(),d12.date) as days from d12 join d11 on d12.prcno=d11.rcno GROUP BY prcno,stkpt HAVING (sum(partreceived)-sum(partissued))>0 ORDER BY d11.pnum) T where stkpt='$cat'";
					$result = $con->query($query);
					$row1 = mysqli_fetch_array($result);
					if($row1['c']>30)
					{
						//echo $row1['c'];
						$query = "SELECT DISTINCT pnum,days FROM (select d11.pnum as pnum,stkpt,prcno as prc,d12.date,sum(partreceived)-sum(partissued) as s ,datediff(NOW(),d12.date) as days from d12 join d11 on d12.prcno=d11.rcno GROUP BY prcno,stkpt HAVING (sum(partreceived)-sum(partissued))>0 ORDER BY d11.pnum) T where stkpt='$cat' AND pnum='$rat' HAVING days>30";
						//echo "SELECT DISTINCT pnum,days FROM (select d11.pnum as pnum,stkpt,prcno as prc,d12.date,sum(partreceived)-sum(partissued) as s ,datediff(NOW(),d12.date) as days from d12 join d11 on d12.prcno=d11.rcno GROUP BY prcno,stkpt HAVING (sum(partreceived)-sum(partissued))>0 ORDER BY d11.pnum) T where stkpt='$cat' AND pnum='$rat' HAVING days>30";
						$result = $con->query($query);
						$row1 = mysqli_fetch_array($result);
						if(mysqli_num_rows($result) <= 0)
						{
							//header("location: inputlink.php?msg=23");
						}
					}
				}
				*/
				...
				if(isset($_GET['cat']))
				{
					...
					{
						$result = mysqli_query($con,"SELECT m12.pnum,m13.foreman FROM `m12` LEFT JOIN m13 ON m12.pnum=m13.pnum WHERE operation='Semifinished3' AND foreman NOT IN (SELECT DISTINCT foreman FROM `d11` JOIN m13 ON d11.pnum=m13.pnum WHERE (d11.rcno LIKE 'B20____%' OR d11.rcno LIKE 'C20____%') AND d11.date<='$dt' AND closedate='0000-00-00')");
					...
					{
						$result = mysqli_query($con,"SELECT m12.pnum,m13.foreman FROM `m12` LEFT JOIN m13 ON m12.pnum=m13.pnum WHERE operation='Semifinished2' AND foreman NOT IN (SELECT DISTINCT foreman FROM `d11` LEFT JOIN m13 ON d11.pnum=m13.pnum WHERE (d11.rcno LIKE 'B20____%' OR d11.rcno LIKE 'C20____%') AND d11.date<='$dt' AND closedate='0000-00-00')");
					...
					if($cat=="Rework")
					{
						$result = mysqli_query($con,"SELECT DISTINCT pnum From m12 where operation='FG For S/C'");
						//echo "SELECT DISTINCT m12.pnum,m13.foreman FROM m12 JOIN m13 ON m12.pnum=m13.pnum WHERE operation='$cat' AND m13.foreman NOT IN (SELECT foreman FROM (SELECT DISTINCT d11.rcno,foreman FROM `d11` LEFT JOIN m13 ON d11.pnum=m13.pnum WHERE d11.rcno LIKE 'E20____%' AND d11.date<='$dt' AND closedate='0000-00-00') AS T1 WHERE foreman!='' GROUP BY foreman HAVING COUNT(*)>10)";
					}
					...
					if($cat=="CLE UNIT 2")
					{
						...
						//$result = mysqli_query($con,"SELECT DISTINCT pnum From m12 where operation='CLE UNIT 2'");
						//$result = mysqli_query($con,"SELECT m12.pnum,m13.foreman FROM `m12` LEFT JOIN m13 ON m12.pnum=m13.pnum WHERE operation='CLE UNIT 2' AND foreman NOT IN (SELECT DISTINCT foreman FROM `d11` JOIN m13 ON d11.pnum=m13.pnum WHERE (d11.rcno LIKE 'B20____%' OR d11.rcno LIKE 'C20____%') AND d11.date<='$dt' AND closedate='0000-00-00')");
						//echo "SELECT m12.pnum,m13.foreman FROM `m12` LEFT JOIN m13 ON m12.pnum=m13.pnum WHERE operation='CLE UNIT 2' AND foreman NOT IN (SELECT DISTINCT foreman FROM `d11` JOIN m13 ON d11.pnum=m13.pnum WHERE (d11.rcno LIKE 'B20____%' OR d11.rcno LIKE 'C20____%') AND d11.date<='$dt' AND closedate='0000-00-00')";
					}
		...
		?>
						...
						<?php	
							if(isset($_GET['rat'])) 
							{
								...
								if($cat=="CLE UNIT 2")
								{
									//$query2 = "select prcno as rcno,sum(partreceived)-sum(partissued)-sum(qtyrejected) as qty from d12 where stkpt='$cat' and pnum='$mat' and prcno in (select DISTINCT d11.rcno from d11 join d12 on d11.rcno=d12.prcno join m13 on d12.pnum=m13.pnum where d12.pnum='$mat' and d11.rcno!='') group by prcno HAVING qty>0";
									//$query2 = "SELECT prcno as rcno,SUM(partreceived)-SUM(partissued) FROM d12 WHERE  prcno IN (SELECT rcno FROM d11 WHERE pnum='$mat' AND operation='To S/C') GROUP BY prcno HAVING SUM(partreceived)-SUM(partissued)>0";
									//$query2 = "SELECT * FROM `d11` WHERE pnum='29192689' AND operation='FG For S/C' AND closedate='0000-00-00'";
			...
			<br>
				<script>
					function reload2(form)
					{
						...
						self.location=<?php if(isset($_GET['cat']))echo"'unit2cle.php?tdate=".$_GET['tdate']."&cat=".$_GET['cat']."&rat=".$_GET['rat']."&mat='";?>+val+'&dc='+s; ;
					}
				</script>
			<div>
			<?php
				...
				if($cat=="FG For Invoicing")
				...
				else
					{
						echo"<label>ISSUANCE RCNO</label>";
							...
							//$query = "Select rcno from cledb where operation='CLE UNIT 2' order by rcno desc";
			...
			<label>AVALABLE QTY</label> 
					<input type="text" readonly="readonly" required  name="available" value="<?php 
						...
						{	
							...
							$rat=$_GET['rat'];
							$cat=$_GET['cat'];
							//$query = "Select sum(partreceived) from d12 where prcno='$mat' and stkpt='FG For S/C'";
							/*$query = "Select sum(partissued) from d12 where rcno='$mat' and stkpt='FG For S/C'";
							$result = $con->query($query);
							$row = mysqli_fetch_row($result);
							$sopr=$row[0];
							//$query1 = "Select sum(partreceived) from d12 where prcno='$mat' and stkpt='FG For S/C'";
							$query1 = "Select sum(partreceived) from d12 where prcno='$mat'";
							$result1 = $con->query($query1);
							...
							*/
							...
							$query1 = "SELECT issued_qty,received_qty,rejected FROM `cledb` WHERE dcnum='$mat' ";
							//echo "SELECT issued_qty,received_qty,rejected FROM `cledb` WHERE dcnum='$mat' ";
							$result1 = $con->query($query1);
							$row1 = mysqli_fetch_array($result1);
							//$issued_qty = $row1['issued_qty'];
							$received_qty = $row1['received_qty'];
							...
							//$avlqty = $issued_qty - ($received_qty + $rejected);
							$avlqty = $sopr - ($received_qty + $rejected);
			...
			<div>
				<label>SELECT PAINT/UNPAINT</label>
