...
if(isset($_POST['submit']))
{
	...
	$q = "select distinct scn from `unit2_dc_det` where dcnum='$dc'";
	$r = $con->query($q);
	if($r === FALSE) { 
		$n1=0;
	}
	...
	{
		$query = "SELECT * FROM `unit2_dc_det` where dcnum='$dc'";
		...
		$query1 = "SELECT * FROM `unit2_dc_det` where dcnum='$dc'";
		...
		echo '<div class="tg-wrap"><table class="tg">
			  <tr>
				<th class="tg-bn4o" colspan="12">GST DELIVERY CHALLAN '.$type.'</th>
			  </tr>
			  <tr>
			  ...
			  <tr>
				<td class="tg-amwm" colspan="12">UNIT - II , 6/164-A,CHETTIPALAYAM ROAD,</td>
			  </tr>
			  <tr>
				<td class="tg-amwm" colspan="12">PAPPAMPATTI,COIMBATORE - 641016. Ph.No. 0422 2680840 , 9659877955</td>
			  </tr>
			  <tr>
				<td class="tg-amwm" colspan="2">GSTIN: 33AACCV3065F1ZL</td>
				<td class="tg-l2oz" colspan="2">MODE OF TRANSPORT</td>
				<td class="tg-9hbo" colspan="8">'.$row['mot'].'</td>
			  </tr>
			  <tr>
				<td class="tg-amwm" colspan="2">DC NUMBER</td>
				<td class="tg-l2oz" colspan="2">VEHICLE NUMBER</td>
				<td class="tg-yw4l" colspan="8">'.$row['vehiclenumber'].'</td>
			  </tr>
			  <tr>
				<td class="tg-ujoh" colspan="2">'.$row['dcnum'].'</td>
				<td class="tg-l2oz" colspan="2">DATE AND TIME OF SUPPLY</td>
				<td class="tg-yw4l" colspan="8">'.date('d-m-Y  # H:i').'</td>
			  </tr>
			  <tr>
				<td class="tg-amwm">DATE:</td>
				<td class="tg-baqh">'.date('d-m-Y').'</td>
				<td class="tg-l2oz" colspan="2">PLACE OF SUPPLY</td>
				<td class="tg-yw4l" colspan="8">'.$row1['scn'].'</td>
			  </tr>
			  <tr>
				<td class="tg-bn4o" colspan="12">DETAILS OF THE RECEIVER</td>
			  </tr>
			  <tr>
				<td class="tg-9hbo" colspan="4">Name:<br>Address:<br><br>State:<br>state code :<br>GSTUnique ID :</td>
				<td class="tg-yw4l" colspan="8">'."VSS U-1".'<br>'."1/89 Ravuthur Pirivu, Kannampalayam. Sulur Coimbatore 641 402".'<br><br>'.$row1['state'].'<br>'.$row1['sc'].'<br>'."33AACCV3065F1ZL".'</td>
			  </tr>
			  <tr>
		...
		while($row = mysqli_fetch_array($result))
		{
			...
			echo '<tr>
				...
				<td class="tg-baqh" colspan="2">'.$row['pn'].$row1['pd'].'</td>
		...
		echo '<tr>
			...
			<td class="tg-amwm" colspan="7">TOTAL</td>
			<td class="tg-amwm">'.$sum.'</td>
		  </tr>
		  <tr>
			<td class="tg-amwm" colspan="12">TOTAL VALUE IN WORDS</td>
		  </tr>
		  <tr>
			<td class="tg-yw4l" colspan="12">'.$a.'</td>
		  </tr>
		   <tr>
			<td class="tg-9hbo" colspan="6">Purpose : meterial sent for '.$row1['operdesc'].' work only</td>
			<td class="tg-9hbo" colspan="6">For Venkateswara Steels &amp; Springs India Pvt Ltd            </td>
		  </tr>
		  <tr>
			<td class="tg-yw4l" colspan="12" rowspan="2"><br>RECEIVED,THE ABOVE GOODS<br><br>CUSTOMER SIGNATURE<br>.</td>
		  </tr>
		  <tr>
		  </tr>
		</table></div>';
