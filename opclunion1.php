...
<body>
	...
	<div  align="center"><br>
		...
		<h4 style="text-align:center"><label> WEEKLY/MONTHLY MATERIAL STOCK PROCESSING FOR ADMIN USE  </label></h4>
	...
	<script>
			function reload(form)
			{
				...
				self.location='opclunion1.php?f='+s0+'&tt='+s1;
			}
	</script>
...
LEFT JOIN(SELECT invpnum AS rminvpnum,category AS invcategory FROM (SELECT DISTINCT pn_st.pnum,invpnum,category FROM pn_st LEFT JOIN rmcategory ON rmcategory.pnum=pn_st.pnum) AS T0 GROUP BY rminvpnum) AS INVRMCAT ON pnum=INVRMCAT.rminvpnum
				...
				while($row = mysqli_fetch_array($result))
				{
					...
					echo"<td>".$row['FCat']."</td>";
					echo"<td>".$row['unit']."</td>";
					echo"<td>".round($row['openingstk'],2)."</td>";
					echo"<td>".round($row['opissqty'],2)."</td>";
					echo"<td>".round($row['weekrec'],2)."</td>";
					echo"<td>".round($row['weekrej'],2)."</td>";
					echo"<td>".round($row['ret'],2)."</td>";
					echo"<td>".round($row['closingstk'],2)."</td>";
