	...
	<div class="divclass">
		<form method="GET">			
			...
			else
			{
				...
				//$query3 = "SELECT stkpt,date,pnum,prcno,qty,datediff(NOW(),date) as days FROM `f_insp` WHERE apby='' AND status='F'";
				$query3 = "SELECT * FROM(SELECT stkpt,date,pnum,prcno,qty,datediff(NOW(),date) as days FROM `f_insp` WHERE apby='' AND status='F') AS TFINSP LEFT JOIN(SELECT rcno,pnum,closedate FROM d11) AS Td11 ON TFINSP.prcno=Td11.rcno WHERE closedate!='0000-00-00'";
				$result3 = $conn->query($query3);
				while($row3 = mysqli_fetch_array($result3))
				{
					echo"<tr><td>".$row3['stkpt']." ( FFINAL APPROVAL PENDING ) "."</td>";
					echo"<td>".$row3['date']."</td>";
					echo"<td>".$row3['pnum']."</td>";
					echo"<td>".$row3['prcno']."</td>";
					echo"<td>".$row3['qty']."</td>";
					echo"<td>".$row3['days']."</td>";
					echo"</tr>";
				}
