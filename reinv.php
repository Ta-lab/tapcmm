	...
	<div>
		...
		<script>
			...
			function reload0(form)
			{
				...
				self.location=<?php echo"'reinv.php?tdate='"?>+p1+'&inv='+p0+'&ccode='+p;
			}
			function reload(form)
			{
				...
				self.location=<?php echo"'reinv.php?tdate='"?>+p1+'&inv='+p0+'&ccode='+p+'&partnumber='+p2;
			}
			function reloadpn(form)
			{
				...
				self.location=<?php echo"'reinv.php?tdate='"?>+p1+'&inv='+p0+'&ccode='+p+'&partnumber='+p2+'&pn='+pn.value;
			}
			function reload1(form)
			{
				...
				self.location=<?php echo"'reinv.php?tdate='"?>+p1+'&inv='+p0+'&ccode='+p+'&partnumber='+p2+'&cname='+p3;
			}
			function reload2(form)
			{
				...
				self.location=<?php echo"'reinv.php?tdate='"?>+p1+'&inv='+p0+'&ccode='+p+'&partnumber='+p2+'&cname='+p3+'&cpo='+p4;
			}
			function preventback()
			{
				window.history.forward();
		...
		<form action="inventry.php" method="post" enctype="multipart/form-data">
			<div id="stylized" class="myform">
				...
				<label>INVOICE NUMBER&nbsp;&nbsp;&nbsp;&nbsp;</label>
					...
					if(isset($_GET['inv']))
					{
						$q = "SELECT invno,invdt FROM inv_correction where apby!='' and status='F' LIMIT 1";
						$r = $con->query($q);
						$fch=$r->fetch_assoc();
						if($_GET['inv']==$fch['invno'])
						{
							echo $_GET['inv'];
						}
						else
					...
					else
					{
						...
						if($c==0)
						{
							header('location: inputlink.php?msg=19');
						}
						$fch=$r->fetch_assoc();
						$invdate=$fch['invdt'];
						echo $fch['invno'];
		...
		?>
				<div class="column">
					...
					<input readonly="readonly" type="date" id="p1" readonly name="tdate" value="<?php
					if(isset($_GET['tdate']))
					{
						echo $_GET['tdate'];
					}
				...
				<div class="column">
					...
					<div class="column">
						...
						<input type="text" required readonly='readonly' id="p7" name="avlqty" min="1"  value="<?php 
							$t="";
								if(isset($_GET['partnumber']) && $_GET['partnumber']!="" && ($dir==0 || isset($_GET['pn']) && $_GET['pn']!="")){
								$rat = $_GET['partnumber'];
								if(isset($_GET['pn']) && $_GET['pn']!="")
								{
									$pnum = $_GET['pn'];
								...
								else
								{
									...
									else
									{
										...
										$query = "SELECT SUM(rem) as stock FROM (SELECT DISTINCT T2.rcno,T2.pnum,T2.date,T2.issqty-IF(T1.received IS NULL,'0',T1.received) as rem,datediff(NOW(),T2.date) as days FROM (SELECT prcno,d12.pnum,SUM(partreceived) as received,SUM(qtyrejected) as rejected FROM `d12` WHERE prcno!='' GROUP BY d12.prcno) AS T1 RIGHT JOIN (SELECT DISTINCT d11.rcno,m14.oper as operation,d11.date,IF(d11.rcno LIKE 'A20%', d12.rm, d12.pnum) as rm,d12.pnum,sum(d12.partissued) as issqty FROM d12 JOIN d11 ON d12.rcno=d11.rcno JOIN m14 on d11.operation=m14.stkpt WHERE d11.rcno!='' AND d11.closedate='0000-00-00' AND d11.operation='FG For S/C' AND d11.pnum='$rat' GROUP BY rcno ORDER BY date) AS T2 ON T1.prcno=T2.rcno GROUP BY T2.rcno order by t2.date,T2.rcno ASC) AS T";
										//echo "SELECT SUM(rem) as stock FROM (SELECT DISTINCT T2.rcno,T2.pnum,T2.date,T2.issqty-IF(T1.received IS NULL,'0',T1.received) as rem,datediff(NOW(),T2.date) as days FROM (SELECT prcno,d12.pnum,SUM(partreceived) as received,SUM(qtyrejected) as rejected FROM `d12` WHERE prcno!='' GROUP BY d12.prcno) AS T1 RIGHT JOIN (SELECT DISTINCT d11.rcno,m14.oper as operation,d11.date,IF(d11.rcno LIKE 'A20%', d12.rm, d12.pnum) as rm,d12.pnum,sum(d12.partissued) as issqty FROM d12 JOIN d11 ON d12.rcno=d11.rcno JOIN m14 on d11.operation=m14.stkpt WHERE d11.rcno!='' AND d11.closedate='0000-00-00' AND d11.operation='FG For S/C' AND d11.pnum='$rat' GROUP BY rcno ORDER BY date) AS T2 ON T1.prcno=T2.rcno GROUP BY T2.rcno order by t2.date,T2.rcno ASC) AS T";
