...
<body>
	...
	<div  align="center"><br>
		...
		<h4 style="text-align:center"><label> WEEKLY MATERIAL PROCESSING SUMMARY BY CATEGORY </label></h4>
	<div style="float:left">
			<a href="inputlink.php"><label>HOME</label></a>
	</div>
	<div style="float:right">
		<input type="button" onclick="tableToExcel('testTable', 'WEEKLY MATERIAL PROCESSING SUMMARY BY CATEGORY')" value="Export to Excel">
	</div>
	...
	<script>
			function reload(form)
			{
				...
				self.location='weekly_mpreport_summary_category.php?f='+s0+'&tt='+s1;
			}
	</script>
	...
	<script>
		function reload1(form)
		{
			...
			self.location='weekly_mpreport_summary_category.php?week='+s4;
		}
	</script>
	...
	<div class="divclass">
			<?php
				...
				$d=$_GET['week'];
				$query1 = "SELECT DISTINCT date_ft FROM `mp_report_summary_category` where week='$d'";
				$result1 = $conn->query($query1);
				$row1 = mysqli_fetch_array($result1);
				
			echo'<table id="testTable" align="center">
				<tr>
					<td colspan="11"><h4>WEEKLY/MONTHLY MATERIAL PROCESSING REPORT FROM '.$row1['date_ft'].' </h4>
				...
				$query = "SELECT * FROM `mp_report_summary_category` where week='$d' ";
				//echo "SELECT * FROM `mp_report_summary_category` where week='$d'";
