...
LEFT JOIN(SELECT invpnum AS rminvpnum,category AS invcategory FROM (SELECT DISTINCT pn_st.pnum,invpnum,category FROM pn_st LEFT JOIN rmcategory ON rmcategory.pnum=pn_st.pnum) AS T0 GROUP BY rminvpnum) AS INVRMCAT ON pnum=INVRMCAT.rminvpnum
				...
				while($row = mysqli_fetch_array($result))
				{
					...
					$dcnum = substr($row['rcno'], 3);
					$query1 = "SELECT dcnum,scn FROM `dc_det` where dcnum='$dcnum' ";
					...
					echo"<td>".$row['FCat']."</td>";
					echo"<td>".round($row['OPENKG'],2)."</td>";  		//OPENING
					echo"<td>".round($row['opissqty_KG'],2)."</td>";	//RECEIVED
					echo"<td>".round($row['weekrec_KG'],2)."</td>";		//OK
					echo"<td>".round($row['weekrej_KG'],2)."</td>";		//SCRAP
					echo"<td>".round($row['ret'],2)."</td>";			//RETURN
					echo"<td>".round($row['closingstk_KG'],2)."</td>";	//CLOSING
					...
					//PROCESSED QTY/ NET RECD QTY % FORMULA : (OK+SCRAP)/(RECEIVED-RETURN)*100
					$processedqty = ($row['weekrec_KG']+$row['weekrej_KG'])/($row['opissqty_KG']-$row['ret'])*100;
					echo"<td>".round($processedqty,0)."</td>";	
					
					//SCRAP/ OK QTY % FORMULA : (SCRAP/OK)*100
					$scrap_per = $row['weekrej_KG']/$row['weekrec_KG']*100; 
					echo"<td>".round($scrap_per,0)."</td>";	
					
					//CLOSING STOCK/ OK QTY % FORMULA : (CLOSING/OK)*100
					$close_per = $row['closingstk_KG']/$row['weekrec_KG']*100; 
					echo"<td>".round($close_per,0)."</td>";	
					
					
					//DIFF/ OK QTY %  FORMULA : (dif/OK)*100
					$diff_per = $dif/$row['weekrec_KG']*100; 
