		...
		<form method="GET">	
			...
			<?php
				...
				if(isset($_GET['pn']) && $_GET['pn']!="")
				{
					...
					$query = "SELECT dcnum,dcdate,scn,DCREPORT.pn,qty,rate,per,Tm13.useage,bom,IF(useage IS NULL,bom,useage) AS TBOM,(qty*IF(useage IS NULL,bom,useage)) AS weight,(qty*rate/per) AS amount,type FROM(select dc_det.dcnum,dc_det.dcdate,dc_det.scn,dc_det.pn,dc_det.qty,dc_det.type from dc_det where dc_det.pn='$p' and dcdate>='$f' and dcdate<='$tt') AS DCREPORT
...
group by dcnum";
				...
				else
				{
					...
					$query = "SELECT dcnum,dcdate,scn,DCREPORT.pn,qty,rate,per,Tm13.useage,bom,IF(useage IS NULL,bom,useage) AS TBOM,(qty*IF(useage IS NULL,bom,useage)) AS weight,(qty*rate/per) AS amount,type FROM(select dc_det.dcnum,dc_det.dcdate,dc_det.scn,dc_det.pn,dc_det.qty,dc_det.type from dc_det where dcdate>='$f' and dcdate<='$tt') AS DCREPORT
