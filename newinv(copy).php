		...
		<script>
			...
			function reload0(form)
			{
				...
				<?php
					...
					if(isset($_GET['ccode'])){	
						$ccode=$_GET['ccode'];
						if( $ccode=="LMW-UNIT-1" || $ccode=="LMW-UNIT-2" || $ccode=="LMW (MTD)" )
						{
					...
					<div class="column">
						...
						<input type="text" required readonly='readonly' id="cmt" name="cmt" min="1"  value="<?php 
								...
								if($c>0)
								{
									...
									$result2 = $con->query("SELECT pnum,obqty,IF(invoiced_qty IS NULL,0,invoiced_qty) AS invoiced_qty FROM(SELECT pnum,SUM(qty) AS obqty FROM `orderbook` WHERE pnum='$pnum') AS ob LEFT JOIN(SELECT pn,SUM(qty) AS invoiced_qty FROM inv_det WHERE pn='$pnum' AND invdt>='2020-02-01') AS invdet ON ob.pnum=invdet.pn");
