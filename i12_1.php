...
.column1
{
	...
	if(isset($_GET['rat']) && $_GET['rat']!="")
	{
		...
		if($rowd['cnc_excep']==1 || $rowd1['c']>0)
		{
			...
			/*$result2 = mysqli_query($con,"SELECT * FROM `rmcategory` WHERE pnum='$t' ");
		...
		else
		{
			...
			if($row['c']>0)
			{
				...
				*/
				...
				//NPD No Lock
				$query9 = "SELECT pnum,foreman FROM m13 WHERE pnum='$t' AND foreman!='NPD'";
				...
				*/
				...
				if($row1['c']>0)
				{
					...
					$query6 = "SELECT * FROM `lock_mechanism` WHERE lock_area='CNC' ";
					$result6 = $con->query($query6);
					$row6 = mysqli_fetch_array($result6);
					$lock_date = $row6['lock_date'];
					$todaydt = date("Y-m-d");
					if($lock_date != $todaydt){
						header("location: reportmail_cnclock.php");
					}else{
						header("location: inputlink.php?msg=10");
					}
				}
			...
			/*$result2 = mysqli_query($con,"SELECT * FROM `opbom` WHERE pnum='$t' ");
			$row2 = mysqli_fetch_array($result2);
	...
	<?php
		...
		$query = mysqli_query($con,"SELECT rcno FROM d11 WHERE operation='Stores' AND rcno LIKE '%A2022%' ORDER BY rcno DESC");
	...
	?>"></h1>
			...
			<div class="column">
			<label>HEAT NO&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
			<input type="text" list="combo-option" name ="hno" required placeholder="Enter Heat number" id="hno" value="<?php
				if(isset($_GET['mat']))
				{
					echo $_GET['mat'];
				}
			...
			<?php					
				...
				<div class="column">
					<label>LOT NO&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
					<input type="text" name ="lno" id="lno" required placeholder="Enter lot number">
				</div>
				<div class="column">
					<label>COIL NO&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
					<input type="text" name ="cno" id="cno" required placeholder="Enter coil number">
				</div>
			<div>
