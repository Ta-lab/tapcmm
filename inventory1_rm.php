...
if(isset($_POST['submit']))
{
	...
	{
		$query9 = "SELECT vehiclenumber FROM dc_det where dcnum='$dc'";
		$result9 = $con->query($query9);
		$row9 = mysqli_fetch_array($result9);
		
		//$query = "SELECT * FROM dc_det where dcnum='$dc'";
		$query = "SELECT * FROM rmdc where dcnum='$dc'";
		...
		$query1 = "SELECT  * FROM storedb.`receipt` WHERE grnnum='$grn'";
		//echo "SELECT  * FROM `receipt` WHERE grnnum='$grn'";
		...
		//$rate=$row1['rate'];
		//$per=$row1['per'];
		$rm = trim($row1['rmdesc'],"- DM");
		$rm1 = trim($row1['rmdesc'],"- DH");
		$rm_sup_name = $row1['sname'];
		//echo $rm;
		//echo $rm_sup_name;
		
		$connec = mysqli_connect('localhost','root','Tamil','erpdb');
		//$connec = mysqli_connect('localhost','root','Tamil','mypcm');
		//$query2 = "SELECT * FROM erpdb.`m21_supplierpartdetail` WHERE internal_rmdesc LIKE '%$rm%' AND scode='$rm_sup_name'";
		$query2 = "SELECT * FROM erpdb.`m21_supplierpartdetail` WHERE internal_rmdesc LIKE '%$rm%' OR internal_rmdesc LIKE '%$rm1%'";
		//echo "SELECT * FROM erpdb.`m21_supplierpartdetail` WHERE internal_rmdesc LIKE '%$rm%' OR internal_rmdesc LIKE '%$rm1%'";
		//echo "SELECT * FROM erpdb.`m21_supplierpartdetail` WHERE internal_rmdesc LIKE '%$rm%' AND scode='$rm_sup_name'";
		//$query2 = "SELECT * FROM `rmprice` WHERE rmdesc LIKE '%$rm%' AND sname='$rm_sup_name'";
		//echo "SELECT * FROM `rmprice` WHERE rmdesc='$rm' AND sname='$rm_sup_name'";
		//echo "SELECT * FROM `m21_supplierpartdetail` WHERE internal_rmdesc='$rm' AND sccode='$rm_sup_name'";
		$result2 = $connec->query($query2);
		$row2 = mysqli_fetch_array($result2);
		$rate = $row2['rate'];
		
		
		$query1 = "SELECT * FROM dcmaster where sccode='$t2'";
		...
		echo '<div class="tg-wrap"><table class="tg">
			  <tr>
				<th class="tg-bn4o" colspan="8">GST DELIVERY CHALLAN '.$type.'</th>
			  </tr>
			  <tr>
				<td class="tg-3b15" colspan="2" rowspan="3"><img src="img/logo1.png" alt="Mountain View" style="width:100%;height:60%;"></img></td>
				<td class="tg-bn4o" colspan="6">VENKATESWARA STEELS AND SPRINGS (INDIA) PVT LTD</td>
			  </tr>
			  <tr>
				<td class="tg-amwm" colspan="6">1/89 Ravuthur Pirivu, Kannampalayam.</td>
			  </tr>
			  <tr>
				<td class="tg-amwm" colspan="6">Sulur Coimbatore 641 402 Ph.No. 0422 2680840 , 9659877955</td>
			  </tr>
			  <tr>
				<td class="tg-amwm" colspan="2">GSTIN: 33AACCV3065F1ZL</td>
				<td class="tg-l2oz" colspan="2">MODE OF TRANSPORT</td>
				<td class="tg-9hbo" colspan="4">'.$row['mot'].'</td>
			  </tr>
			  <tr>
				<td class="tg-amwm" colspan="2">DC NUMBER</td>
				<td class="tg-l2oz" colspan="2">VEHICLE NUMBER</td>
				<td class="tg-yw4l" colspan="4">'.$row9['vehiclenumber'].'</td>
			  </tr>
			  <tr>
				<td class="tg-ujoh" colspan="2">'.$row['dcnum'].'</td>
				<td class="tg-l2oz" colspan="2">DATE AND TIME OF SUPPLY</td>
				<td class="tg-yw4l" colspan="4">'.date('d-m-Y  # H:i').'</td>
			  </tr>
			  <tr>
				<td class="tg-amwm">DATE:</td>
				<td class="tg-baqh">'.date('d-m-Y').'</td>
				<td class="tg-l2oz" colspan="2">PLACE OF SUPPLY</td>
				<td class="tg-yw4l" colspan="4">'.$row1['scn'].'</td>
			  </tr>
			  <tr>
				<td class="tg-bn4o" colspan="8">DETAILS OF THE RECEIVER</td>
			  </tr>
			  <tr>
				<td class="tg-9hbo" colspan="2">Name                                         :<br>Address                                    :<br><br>State                                          :<br>state code                                :<br>GSTUnique ID                          :</td>
				<td class="tg-yw4l" colspan="6">'.$row['scn'].'<br>'.$row1['sca1'].$row1['sca2'].$row1['sca3'].'<br><br>'.$row1['state'].'<br>'.$row1['sc'].'<br>'.$row1['gst'].'</td>
			  </tr>
			  <tr>
				<td class="tg-amwm">S.NO</td>
				<td class="tg-amwm" colspan="2">ITEM DESCRIPTION</td>
		...
		while($row = mysqli_fetch_array($result))
		{
			...
			echo '<tr>
				...
				<td class="tg-baqh" colspan="2">'.$row['rm'].'</td>
				<td class="tg-baqh">'.$row2['hsnc'].'</td>
				<td class="tg-baqh">'.$row['qty'].'</td>
				<td class="tg-baqh">KG</td>
				<td class="tg-baqh">'.$rate.'</td>
				<td class="tg-baqh">'.($row['qty']*$rate).'</td>
			  </tr>';
			$sum=$sum+($row['qty']*$rate);
		}
		while($s<10)
		{
			$s=$s+1;
		...
		echo '<tr>
			...
			<td class="tg-amwm" colspan="3">TOTAL</td>
			<td class="tg-amwm">'.$sum.'</td>
		  </tr>
		  <tr>
			<td class="tg-amwm" colspan="8">TOTAL VALUE IN WORS</td>
		  </tr>
		  <tr>
			<td class="tg-yw4l" colspan="8">'.$a.'</td>
		  </tr>
		   <tr>
			<td class="tg-9hbo" colspan="4">Purpose : </td>
			<td class="tg-9hbo" colspan="4"></td>
		  </tr>
		  <tr>
			<td class="tg-yw4l" colspan="8" rowspan="2"><br>RECEIVED,THE ABOVE GOODS<br><br>CUSTOMER SIGNATURE<br>.</td>
		  </tr>
		  <tr>
		  </tr>
		</table></div>';
