...
$message .= '<tr><th> AREA </th><th> FOREMAN </th><th> CRITERIA </th><th> No Of ROUTE CARD </th><th> STATUS </th><th> ABOVE 10 days </th></tr><tr>';
$message .= '<tr><td> CNC AREA </td><td>RAVI & PRABHU </td><td> MAXIMUM 125 </td><td>'.$cnc.'</td><td>'.$cs.'</td><td>'.$cc.'</td></tr>';
$message .= '<tr><td> SHEARING / SHEET CUTTING </td><td>RAVI & PRABHU </td><td> MAXIMUM 25 </td><td>'.$sas.'</td><td>'.$scs.'</td><td>'.$sc.'</td></tr>';
$message .= "</table>";
$dt=date('Y-m-d',strtotime('-6 days'));
...
$message .= '<tr><th> AREA </th><th> FOREMAN </th><th> CRITERIA </th><th> NO OF DC </th><th> STATUS </th></tr><tr>';
$message .= '<tr><td> SUB-CONTRACT </td><td> GURUMOORTHY </td><td> DC > 30 Days Must Be Zero </td><td>'.$row['c'].'</td><td>'.$sc.'</td></tr>';
$message .= '<tr><td> ALFA + N-IND + PRIM </td><td> SARAVANAN </td><td> STOCK > 5 Days Must Be Zero </td><td>'.$row1['c'].'</td><td>'.$dc.'</td></tr>';
$message .= "</table>";
...
$result = $con->query($query);
$row = mysqli_fetch_array($result);
$query1 = "SELECT COUNT(*) AS c FROM (select distinct(PNUM) as pnum,prcno as prc,date,sum(partreceived)-sum(partissued) as s ,datediff(NOW(),date) as days from d12 where pnum!='' and stkpt='FG For S/C' GROUP BY prcno HAVING (sum(partreceived)-sum(partissued))>0 AND days>1) AS T";
$result1 = $con->query($query1);
$row1 = mysqli_fetch_array($result1);
$inv="OK";
$dc="OK";
...
$message .= '<tr><td> FG FOR S/C </td><td> SHOULD BE ZERO </td><td>'.$row1['c'].'</td><td>'.$dc.'</td></tr>';
$message .= "</table>";
...
$mail->Username = 'enterpriseexcell@gmail.com';


// your password must be enclosed in single quotes
$mail->Password = '9659008949';

// add a subject line
$mail->Subject = 'REPORT ON -'.date("D-M-Y ");

// Sender email address and name
$mail->SetFrom('enterpriseexcell@gmail.com', 'Narayanan');
