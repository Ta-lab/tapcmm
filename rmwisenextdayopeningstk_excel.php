	...
	if(1)
	{
		...
		$style = array(
		  'alignment' => array(
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('C1', ' OPENING STOCK ON '.date("d-m-Y"));
		...
		while($row = mysqli_fetch_array($result))
		{
			...
			mysqli_query($conn,"INSERT INTO nextdayopening(edate,date,operation,category,pnum,rcno,qty,unit) VALUES('$edate','$date','$operation','$category','$pnum','$rcno','$qty','$unit')");
		...
		*/
		...
		$objPHPExcel->getActiveSheet()->setTitle("OPENING STOCK ON ".date("d-m-Y"));
		...
		*/
		...
		$objWriter->save('report\MATERIALRECONCILATION\NEXTDAY OPENING STOCK-'.date("Y-m-d").'.xlsx');
		$objWriter->save('D:\REPORT\MATERIALRECONCILATION\NEXTDAY OPENING STOCK-'.date("Y-m-d").'.xlsx');
		$objPHPExcel->setActiveSheetIndex(0);
