    ...
    setTimeout(typeWriter, speed);
		...
		<script>
			...
			function reload0(form)
			{
				...
				self.location=<?php echo"'newinv1.php?tdate='"?>+p1+'&inv='+p0+'&ccode='+p+'&appno='+appno;
			}
			function reload(form)
			{
				...
				var appno = document.getElementById("appno").value;
				self.location=<?php echo"'newinv1.php?tdate='"?>+p1+'&inv='+p0+'&ccode='+p+'&partnumber='+p2+'&appno='+appno;
			}
			function reloadpn(form)
			{
				...
				var appno = document.getElementById("appno").value;
				self.location=<?php echo"'newinv1.php?tdate='"?>+p1+'&inv='+p0+'&ccode='+p+'&partnumber='+p2+'&pn='+pn.value+'&appno='+appno;
				
				<?php
					
					if(isset($_GET['partnumber']) && isset($_GET['appno'])){
					...
					if($row1['pnum']==$pno){						
						$query2 = "SELECT * FROM npd_invoicing WHERE appno='$appno'";
						$result2 = $con->query($query2);
						$row2 = mysqli_fetch_array($result2);
						if($row2['pnum']!=$pno){
							header("location: npd_inv_app.php?msg=1");
						}
						else if($row2['pnum']!=$pno && $row2['approvedby']=='')
						{
							header("location: npd_inv_app.php?msg=2");
						}
			...
			function reloadsc(form)
			{
				...
				if (document.getElementById("pn") !=null) {
					var p3 = document.getElementById("pn").value;
					self.location=<?php echo"'newinv1.php?tdate='"?>+p1+'&inv='+p0+'&ccode='+p+'&partnumber='+p2+'&pn='+p3+'&sc='+p4+'&appno='+appno;
				}
				else
				{
					self.location=<?php echo"'newinv1.php?tdate='"?>+p1+'&inv='+p0+'&ccode='+p+'&partnumber='+p2+'&sc='+p4+'&appno='+appno;
				}
			}
			function reload1(form)
			{
				...
				var appno = document.getElementById("appno").value;				
				self.location=<?php echo"'newinv1.php?tdate='"?>+p1+'&inv='+p0+'&ccode='+p+'&partnumber='+p2+'&cname='+p3+'&appno='+appno;
			}
			function reload2(form)
			{
				...
				var appno = document.getElementById("appno").value;
				self.location=<?php echo"'newinv1.php?tdate='"?>+p1+'&inv='+p0+'&ccode='+p+'&partnumber='+p2+'&cname='+p3+'&cpo='+p4+'&appno='+appno;
			}
		...
		?>
				...
				<div class="column">
						...
						<input type="text"  class='s' required onchange=reload(this.form) id="p2" name="pn" list="partlist" 
								value="<?php 
					...
					if(isset($_GET['appno'])){
						$appno=$_GET['appno'];
						$result2 = $con->query("SELECT * FROM `npd_invoicing` WHERE appno='$appno'");
						$row = mysqli_fetch_array($result2);
						$qty=$row['reqqty'];			
					}
					?>
					<div class="column">
						...
						<input type="number" required <?php if($t==""){echo "readonly";} ?> id="p5" min="<?php if($t==""){echo 0;}else{echo 1;} ?>" max="<?php if($qty==""){echo 0;}else{echo $qty;} ?>" required name="tiqty">
					...
					<div class="column">
						...
						<input readonly="readonly" type="number" id="appno" name="appno" value="<?php
						if(isset($_GET[	'appno']))
						{
							echo $_GET['appno'];
						}
