...
{
	$cori="";$inum = $_POST['inum'];$ccode = $_POST['cc'];/*$msg = $_POST['msg'];*/
	//$tdate = date('d-m-Y', strtotime($_POST['tdate']));
	$tdate = $_POST['tdate'];
	$pn = $_POST['pn'];$cname = $_POST['cname'];$cpono = $_POST['cpono'];$tiqty = $_POST['tiqty'];$mod = $_POST['mod'];
	$con = mysqli_connect('localhost','root','Tamil','mypcm');$q = "select * from invmaster where pn='$pn' and ccode='$ccode'";
	...
	$six=number_format((float)((($rate*$tiqty)/$fch['per'])+(round(($rate*$tiqty*$fch['pc'])/100,2))),2, '.', '');
	...
	$uom=$fch['uom'];
	$five=number_format((float)(($rate*$tiqty*$fch['pc'])/100),2, '.', '');
	...
	$whole = floor($amtwithtcs);
	$fraction = $amtwithtcs - $whole;
	$fraction=$fraction*100;
	$rup=ROUND($fraction);
	$pai=floor($amtwithtcs);
	...
	mysqli_query($con, "INSERT INTO `inv_det` (`invno`, `invdt`, `invt`, `ccode` , `cname`, `cname1`, `cadd1`, `cadd2`, `cadd3`, `cgstno`, `dtname`, `dtname1`, `dtadd1`, `dtadd2`, `dtadd3`, `dtgstno`, `cpono`, `cpodt`, `mot`, `vc`, `ewbno`, `pn`, `pd`, `hsnc`, `poino`, `rate`, `per`, `uom`, `qty`, `pc`, `pcamt`, `taxgoods`, `cigst`, `sgst`, `cigstamt`, `sgstamt`, `taxtotal`, `totcigstamt`, `totsgstamt`, `invtotal`, `inwords`,`inwords1`, `cori`, `r1`, `r2`, `r3`, `r4`, `r5` , `sup` , `ok` , `print` , `pan_no` , `tcs` , `tcspercentage`) VALUES 
	('$inum','$tdate','".date("H:i")."', '$ccode' ,'$cname1','$cname2', '$cadd1', '$cadd2', '$cadd3', '$cgstin', '$dtname', '$dtname1', '$dtadd1', '$dtadd2', '$dtadd3', '$dtgstin', '$cpono', '$cpodt', '$mod', '$vc', '', '$pn', '$pd', '$hsnc', '$poino', '$rate', '$perdisplay', '$uom', '$tiqty', '$pc', '$five1', '$six', '$sev1', '$egt1', '$vsev1', '$vegt1', '$six', '$vsev1', '$vegt1', '$amtwithtcs', '$a1', '$t1', '$cori' , '$r1', '$r2', '$r3', '$r4', '$r5' , '1' , 'T' , 'F' , '$pan_no' , '$tcs_amt' , '$tcspercentage')");
	$result = $con->query("SELECT pnum FROM `pn_st` WHERE invpnum='$pn' and n_iter=1");
	...
	$result = $con->query("SELECT pnum FROM `pn_st` WHERE invpnum='$pn' group by n_iter");
	if($result === FALSE) { 
		$n1=0;
	}
	else
...
<col style="width: 138.66667px">
  ...
  <tr>
    ...
    <td class="tg-yw4l"><?php echo $rate; ?></td>
  ...
  <tr>
    ...
    <td class="tg-yw4l"><?php echo round(($rate*$tiqty*$fch['pc'])/100,2); ?></td>
  ...
  <tr>
	<td class="tg-yw4l" colspan="3"></td>
	<td class="tg-9hbo" colspan="2">TCS 0.075%</td>
  ...
  <tr>
    <td class="tg-yw4l" colspan="3"></td>
    <td class="tg-9hbo">TOTAL INVOICE VALUE : RS.</td>
    <td class="tg-yw4l"><?php echo $amtwithtcs; ?></td>
    <td class="tg-yw4l"></td>
    <td class="tg-yw4l"></td>
    <td class="tg-yw4l"></td>
  </tr>
...
<form method="POST" action="<?php echo "inputlink.php";?>">
<div>
	<?php
	...
	if($n>1)
	{
		echo '<label for="operation">Part Number</label>';
		echo "<select name ='part' required id='part'>";
		echo "<option value=''>Choose the partnumber</option>";
