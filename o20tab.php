...
<body>
	...
	if(isset($_GET['pnum']))
	{
		...
		if($c>1)
		{
			...
			//below query without alfa stock(please use if alfa stock no need)
		...
		while($r = mysqli_fetch_array($result))
		{
			...
			//Display Final Inspection Approval RCs
			$query4 = "SELECT stkpt,pnum,prcno,SUM(qty) AS f_qty FROM `f_insp` WHERE pnum='$pnum' AND apby='' AND status='F' GROUP BY prcno";
			$result4 = $con->query($query4);
			while($row3 = mysqli_fetch_array($result4))
			{
				echo"<tr><td $st>".$row3['stkpt']." (FINAL APPROVAL PENDING)"."</td>";
				echo"<td $st>".""."</td>";
				echo"<td $st>".$row3['pnum']."</td>";
				echo"<td $st>".$row3['prcno']."</td>";
				echo"<td $st>".round($row3['f_qty'],2)."</td>";
				echo"<td $st>Nos</td>";
				echo"</tr>";
			}
