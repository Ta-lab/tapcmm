				...
				<div class="column">
					...
					<div class="column">
						...
						<input type="text" required readonly='readonly' id="cmt" name="cmt" min="1"  value="<?php 
								...
								$result1 = $con->query("SELECT pnum,SUM(qty)-SUM(invstatus) AS cmt FROM `orderbook` WHERE pnum='$pnum'");
								$c = $result1->num_rows;
								if($c>0)
								{
									$result2 = $con->query("SELECT pnum,SUM(qty)-SUM(invstatus) AS cmt FROM `orderbook` WHERE pnum='$pnum'");
									$row = mysqli_fetch_array($result2);
									$cmt=$row['cmt'];
									if($cmt=="")
									{
