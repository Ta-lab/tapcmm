...
{
	//if($_SESSION['access']=="FG For S/C" || $_SESSION['access']=="FG For Invoicing" || $_SESSION['access']=="ALL" || $_SESSION['user']=="133")
...
<div style="float:right">
	...
	<h4 style="text-align:center"><label>VSS UNIT 2 PART RECEIPT ENTRY</label></h4>
	<div class="divclass">
	<script>
	...
	?>
		<form method="POST" action='unit2receiving_entry_db.php'>
			<div>
				<label>DATE</label>
		...
		<script>
			function reload0(form)
			{
				...
				self.location=<?php echo"'unit2receiving_entry.php?tdate='"?>+p1+'&mat='+p2;
			...
			<script>
				function reload(form)
				{
					...
					window.location='unit2receiving_entry.php?tdate='+s.value+'&mat='+p.value+'&cat='+val;
				}
			</script>
			<br>
			<div>
				...
				<select name ='prcno' id='prcno' required onchange='reload1(this.form)'>
					...
					<?php
						if(isset($_GET['cat'])) 
						{
							...
							{
								echo '<script>alert("for Geeks")</script>';
							...
							else if($cat=="FG For S/C" && $_SESSION['user']=="133" || $_SESSION['user']=="123")
							{
								//echo '<script>alert("Geeks for Geeks")</script>';
								
								//$query7 = "SELECT sccode,pn FROM `dcmaster` WHERE sccode='CLE' AND pn='$mat'";
								...
								*/
								/*//if($mat==$clepart)
								{
									echo '<script>alert("CLE")</script>';
  
									$query2 = "SELECT cledcnum AS rcno FROM(SELECT DISTINCT d11.rcno FROM d11 WHERE d11.pnum='$mat' and d11.rcno!='' and d11.operation!='FG For Invoicing' AND d11.operation='FG For S/C' AND d11.closedate='0000-00-00' ) AS Td11 
								...
								//else
								{
									...
									//echo '<script>alert("or Geeks")</script>';
									//else if($cat=="FG For S/C" && $_SESSION['user']=="133" || $_SESSION['user']=="100" || $_SESSION['user']=="123")
									//{
									//$query2 = "SELECT DISTINCT d11.rcno FROM d11 WHERE d11.pnum='$mat' and d11.rcno!='' and d11.operation!='FG For Invoicing' AND d11.operation='FG For S/C' AND d11.closedate='0000-00-00'";
								
									$query2 = "SELECT * FROM(SELECT DISTINCT d11.rcno FROM d11 WHERE d11.pnum='$mat' and d11.rcno!='' and d11.operation!='FG For Invoicing' AND d11.operation='FG For S/C' AND d11.closedate='0000-00-00' AND d11.date>='2022-04-26') AS Td11
											LEFT JOIN(SELECT dcnum,scn FROM dc_det) AS TDCDET ON Td11.rcno=CONCAT('DC-',TDCDET.dcnum) WHERE scn='VSS U-2'";
									$rfrc = $con->query($query2);
								}
							}
			...
			<script>
				function reload1(form)
				{
					...
					self.location=<?php if(isset($_GET['cat']))echo"'unit2receiving_entry.php?tdate=".$_GET['tdate']."&mat=".$_GET['mat']."&cat=".$_GET['cat']."&rat='";?> + val ;
				}
			</script>
						...
						echo"<tr>";
							...
							//echo"<td id='$t'>";
								if(isset($_GET['rat']))
								{
									...
									$query3 = "SELECT * FROM `unit2receiveentry` WHERE dcnum='$rat' ";
									...
									//$query6 = "SELECT * FROM `cledb` WHERE dcnum='$rat' ";
									$query6 = "SELECT SUM(issued_qty) AS issued_qty,SUM(received_qty) AS received_qty FROM `cledb` WHERE dcnum='$rat' ";
									...
									//$result1 = $con->query("SELECT sum(partreceived) as rec,sum(partissued) AS iss,sum(qtyrejected) as rej FROM d12 where prcno='$rat' AND rcno!='' ");
									$temp2=mysqli_fetch_array($result1);
									if($temp2['rec']!='')
									{
										echo $temp2['rec'] + $row3['total_rec_qty'] - $row5['invqty'] ;
										//echo $temp2['rec'] + $row3['total_rec_qty'] + $row6['issued_qty'] - $row6['received_qty'] - $row5['invqty'] ;
									...
									echo"<td id='2' style='display:none'>";
									if($temp2['rej']!='')
									{
										echo $temp2['rej'];
									}
							...
							echo"<td id='3' style='display:none'>";
								if(isset($_GET['rat'])) 
								{
									$rat=$_GET['rat'];
									$r=substr($rat,0,1);
									...
									else
									{
										...
										//$avl =$avl+($temp2['rec']+$temp2['rej']+$row3['total_rec_qty']+$row6['issued_qty']-$row6['received_qty']-$row5['invqty']);
										$tm =  $tm + $avl;
										echo round($na-$avl,2);
									}
								}
