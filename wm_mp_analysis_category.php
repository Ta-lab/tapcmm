...
{
	if($_SESSION['user']=="123" || $_SESSION['user']=="100" || $_SESSION['user']=="130")
	{
		$id=$_SESSION['user'];
		$activity="WMMPREPORT";
...
<body>
	...
	<div  align="center"><br>
		...
		<h4 style="text-align:center"><label> WEEKLY/MONTHLY MATERIAL PROCESSING REPORT BY RM CATEGORY </label></h4>
	<div style="float:left">
			<a href="inputlink.php"><label>HOME</label></a>
	</div>
	<div style="float:right">
		<input type="button" onclick="tableToExcel('testTable', 'WEEKLY/MONTHLY MATERIAL PROCESSING')" value="Export to Excel">
	</div>
	...
	<script>
			function reload(form)
			{
				...
				self.location='wm_mp_analysis_category.php?f='+s0+'&tt='+s1;
			}
	</script>
	...
	<div class="divclass">
			...
			echo'<table id="testTable" align="center">
				...
				<td colspan="10"><h4>WEEKLY/MONTHLY MATERIAL PROCESSING REPORT FROM '.date('d-m-Y',strtotime($f)).' TO '.date('d-m-Y',strtotime($tt)).' </h4>
				<td colspan="4"><h4> EFFICIENCY PARAMETERS </h4></td>
				
				<tr>
					<th>OPERATION|FOREMAN</th>
					<th>RM CATEGORY</th>
					...
					<th>SHOULD BE CLOSING STOCK(KG)</th>
					<th>DIFFERENCE(KG)</th>
...
LEFT JOIN(SELECT invpnum AS rminvpnum,category AS invcategory FROM (SELECT DISTINCT pn_st.pnum,invpnum,category FROM pn_st LEFT JOIN rmcategory ON rmcategory.pnum=pn_st.pnum) AS T0 GROUP BY rminvpnum) AS INVRMCAT ON pnum=INVRMCAT.rminvpnum
...
) AS FINISHQ ) AS PFINALQ) AS FULLFINISHQ GROUP BY FCat ORDER BY FCat";
				...
				while($row = mysqli_fetch_array($result))
				{
					...
					echo"<td>"."ALL AREAS"."</td>";
					echo"<td>".$row['FCat']."</td>";
					echo"<td>".round($row['OPKG'],2)."</td>";         //OPENING
					echo"<td>".round($row['OPISSKG'],2)."</td>";      //RECEIVED 
					echo"<td>".round($row['WEEKRECKG'],2)."</td>";	  //OK QTY	
					echo"<td>".round($row['WEEKREJKG'],2)."</td>";	  //SCRAP
					echo"<td>".round($row['RETKG'],2)."</td>";		  //RETURN
					echo"<td>".round($row['CLKG'],2)."</td>";		  //CLOSING
					
					$sh=$row['OPKG']+$row['OPISSKG']-$row['RETKG']-$row['WEEKRECKG']-$row['WEEKREJKG'];
					...
					/*//PROCESSED QTY/ NET RECD QTY %
					$rec_ret= $row['OPISSKG']-$row['RETKG'];
					if($rec_ret<=0)
					{
						echo"<td>"."0"."</td>";
					}
					...
					{
						$processedqty = ($row['WEEKRECKG']+$row['WEEKREJKG'])/($row['OPISSKG']-$row['RETKG'])*100;
						echo"<td>".round($processedqty,0)."</td>";	
					}
					...
					{
						$scrap_per = $row['WEEKREJKG']/$row['WEEKRECKG']*100; 
						echo"<td>".round($scrap_per,0)."</td>";	
					}
					
					//CLOSING STOCK/ OK QTY %
					if($row['WEEKRECKG']<=0)
					{
						echo"<td>"."0"."</td>";
					}
					...
					{
						$close_per = $row['CLKG']/$row['WEEKRECKG']*100; 
						echo"<td>".round($close_per,0)."</td>";	
					}
					...
					{
						$diff_per = $dif/$row['WEEKRECKG']*100; 
						echo"<td>".round($diff_per,1)."</td>";	
					}
					*/
