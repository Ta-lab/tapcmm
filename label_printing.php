...
<div style="float:left">
	...
	<div align="right">
		...
		<h4 style="text-align:center"><label>HEAT LOT COIL LABEL PRINTING</label></h4>
		<div class="divclass">
	<form action="label_print_serv.php"  method="post">		
	<br>
	<?php
		$ccode="";$fin="";$pnum="";$rcno="";
		$con = mysqli_connect('localhost','root','Tamil','mypcm');
			...
			function track($rcno)
			{
				...
				do{
					...
					if($row2['stkpt']!="To S/C" && $row2['stkpt']!="From S/C")
					{
						...
						if(substr($row2['rcno'],0,1)=="A")
						{
							$query4 = "select DISTINCT heat,lot,coil from d12 where rcno='$temp'";
							...
							//lot no
							if (strpos($GLOBALS['temprc1'], $tempgrn1 ) !== false) {
							...
							else
							{
								if($GLOBALS['temprc1']=="")
								{
									$GLOBALS['temprc1']=$tempgrn1;	
								}
								else
								{
									$GLOBALS['temprc1']=$GLOBALS['temprc1'].','.$tempgrn1;
								}
							}
							
							//coil no
							if (strpos($GLOBALS['temprc2'], $tempgrn2 ) !== false) {
							...
							else
							{
								if($GLOBALS['temprc2']=="")
								{
									$GLOBALS['temprc2']=$tempgrn2;
								}
								else
								{
									$GLOBALS['temprc2']=$GLOBALS['temprc2'].','.$tempgrn2;
								}
							}
	...
	<script>
			function reload(form)
			{
				...
				self.location=<?php echo"'label_printing.php?invno='"?>+p4;
			}
	</script>
	<div>
		<label>INVOICE NO / RC NO</label>
	...
	<div>
		...
		<input type="text" required readonly name="bat1" value="<?php if(isset($_GET['invno'])){echo $batch1;} ?>"/>
	</div>
	<br>
	<div>
		<label>COIL NUMBER</label>
		<input type="text" required readonly name="bat2" value="<?php if(isset($_GET['invno'])){echo $batch2;} ?>"/>
	</div>
	<br>
	<div>
	...
	<br>
	<div align="right">
		<input type="checkbox" id="crack_check" name="crack_check" required value="100%">
		<label>CRACK CHECKED</label>
	</div>
	<br>
	<br>
