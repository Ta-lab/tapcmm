...
<body style="background-image: url('img/6.jpg');">
	...
	<script>
		...
		function reloadpn(form)
		{
			...
			var p2 = document.getElementById("p2").value;
			var p3 = document.getElementById("p3").value;
			var p4 = document.getElementById("pn").value;
			self.location=<?php echo"'alfatest.php?tdate='"?>+p1+'&dc='+p0+'&sccode='+p2+'&partnumber='+p3+'&iss='+p+'&pn_pick='+p4;
		}
			...
			<div id="stylized" class="myform"><br>
						...
						$c = $res->num_rows;
							if($c>1)
							{
								...
								echo '<div class="column">
									...
									<input type="text" style="width: 60%;" id="pn" list="pnlist" required onchange=reloadpn(this.form) name="pn_pick"  value="'.$tmp.'"/></div>';
							}
						}
					?>
				...
				<script>
					function reload2(form)
					{
						...
						var p3 = document.getElementById("p3").value;
						var p4 = document.getElementById("pn").value;
						self.location=<?php echo"'alfatest.php?tdate='"?>+p1+'&dc='+p0+'&sccode='+p2+'&partnumber='+p3+'&iss='+p+'&mat='+val+'&pn_pick='+p4;
					}
				</script>
				
				<?php				
					if(isset($_GET['iss']) && $_GET['iss']=='FG For S/C' && isset($_GET['sccode']) && $_GET['sccode']!="" && isset($_GET['partnumber']) && $_GET['partnumber']!="" || $cnt>0)
					{
						$rat = $_GET['partnumber'];
						//echo $rat;
							if(isset($_GET['pn_pick']) && $_GET['pn_pick']!=""){
								$rat = $_GET['pn_pick'];
							}
						$oper = $_GET['sccode'];
						...
						else
						{
							...
							$c = $result1->num_rows;
							if($c==1 || $cnt>0 || (isset($_GET['pn_pick']) && $_GET['pn_pick']!=""))
							{//echo "hi";
								do{
									$pnum=$row['pnum'];
									if(isset($_GET['pn_pick']) && $_GET['pn_pick']!="")
				...
				<?php
					if(isset($_GET['iss']) && $_GET['iss']=='FG For S/C' &&isset($_GET['pn_pick']) && $_GET['pn_pick']!="" || isset($_GET['partnumber']) && $_GET['partnumber']!="" || $cnt>0)
					{
				...
				<?php
					if($max>=1)
					{
						echo "<br><br><br><div><input type='submit' name='alfaok' value='ENTER'></div>";
					}
				?>
