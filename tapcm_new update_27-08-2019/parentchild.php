...
if(isset($_POST['alfaok']))
{
	...
	else
	{
		...
		else if($n1>=1)
		{
			...
			if(isset($pn_pick)){
				$rat=$_POST['pn_pick']; //pick from pnum
				$pnum=$_POST['pn'];		
				header("location: i12_4.php?tdate=$tdate&cat=$s&rat=$rat&dc=$dt&tiqty=$tiqty&rem=$tiqty&ino=1&i=$iter&pnum=$pnum");
			}
			else{
			$rat=$_POST['pn'];
			$pnum=$_POST['pn'];
