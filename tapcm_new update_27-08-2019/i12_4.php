...
<script>
	function preventback()
	{
		...
		if(isset($_POST['submit'])){
			...
			$i = $_POST['iter'];
			$tiqty = $_POST['tiqty'];
			header("Location: i12_2db.php?tdate=$tdate&prcno=$prcno&operation=$stkpt&ircn=$ircn&pnum=$pnum&iss=$iss&ino=$ino&i=$i&tiqty=$tiqty&pnum1=$pnum1");
		}
		?>
		<?php
		$con=mysqli_connect('localhost','root','Tamil','mypcm');
		...
		if(isset($_POST['sub']))
		{
			...
			if($ino!=1)
			{
				mysqli_query($con,"INSERT INTO d12 (date,stkpt,pnum,prcno,partreceived,username,ip) VALUES ('".date('Y-m-d')."','FG For Invoicing','$pnum1','$ircn','$iss','$u','$ip')");
				mysqli_query($con,"INSERT INTO d11(operation,date,rcno) VALUES('FG For Invoicing','".date('Y-m-d')."','AF'.'$ircn')");
				mysqli_query($con,"INSERT INTO d12 (date,stkpt,pnum,rcno,prcno,partissued,username,ip) VALUES ('".date('Y-m-d')."','FG For Invoicing','$pnum1','AF$ircn','$ircn','$iss','$u','$ip')");
			}
			header("location: i12_4.php?tdate=$tdate&cat=$stkpt&rat=$pnum&dc=$ircn&tiqty=$tiqty&rem=$rem&ino=$ino&i=$i&pnum=$pnum1");
		}
		?>
		<form method="POST" action='i12_4.php'>	
			...
			<label>PART NUMBER_1</label>
			<input type="text" list="combo-options" name ="pnum1" id="pnum1" readonly="readonly" value="<?php
				if(isset($_GET['pnum']))
				{
					echo $_GET['pnum'];
				}
			...
			<div>
				...
				<select name ="prcno" id="prcno" onchange="reload2(this.form)">
						<?php	
							if(isset($_GET['rat']) || isset($_GET['pnum'])) 
							{
			...
			<br>
				<script>
					function reload2(form)
					{
						...
						self.location=<?php if(isset($_GET['cat']))echo"'i12_4.php?ino=".$_GET['ino']."&i=".$_GET['i']."&tdate=".$_GET['tdate']."&cat=".$_GET['cat']."&rat=".$_GET['rat']."&tiqty=".$_GET['tiqty']."&pnum=".$_GET['pnum']."&rem=".$_GET['rem']."&mat='";?>+val+'&dc='+s; ;
					}
				</script>
			<div>
