...
<div style="float:right">
	...
	<h4 style="text-align:center"><label>SUB CONTRACT DC / CLE RC REJECTION ENTRY</label></h4>
	<div class="divclass">
	<script>
	...
	?>
		<form method="POST" action='subcontract_rej_entry_db.php'>
			<div>
				<label>DATE</label>
		...
		<script>
			function reload0(form)
			{
				...
				self.location=<?php echo"'subcontract_rej_entry.php?tdate='"?>+p1+'&mat='+p2;
			...
			<div>
				...
				<select  name ="operation" required onchange="reload(this.form)">
					...
					<?php 	 
							...
							{
								$mat=$_GET['mat'];
								$result = $con->query("SELECT operation FROM m11 where opertype='stocking point' and operation != 'Stores' and operation='FG For S/C' OR operation='CLE UNIT 2'");
			...
			<script>
				function reload(form)
				{
					...
					window.location='subcontract_rej_entry.php?tdate='+s.value+'&mat='+p.value+'&cat='+val;
				}
			</script>
			<br>
			<div>
				<label>RC/DC NUMBER</label>
				<select name ='prcno' id='prcno' required onchange='reload1(this.form)'>
					...
					<?php
						if(isset($_GET['cat'])) 
						{
							...
							if($cat=="FG For S/C" && $_SESSION['user']=="133" || $_SESSION['user']=="100" || $_SESSION['user']=="123")
							{
								//$query2 = "SELECT DISTINCT d11.rcno FROM d11 WHERE d11.pnum='$mat' and d11.rcno!='' and d11.operation!='FG For Invoicing' AND d11.operation='FG For S/C' AND d11.closedate='0000-00-00'";
			...
			<script>
				function reload1(form)
				{
					...
					self.location=<?php if(isset($_GET['cat']))echo"'subcontract_rej_entry.php?tdate=".$_GET['tdate']."&mat=".$_GET['mat']."&cat=".$_GET['cat']."&rat='";?> + val ;
				}
			</script>
