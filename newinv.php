		...
		<script>
			...
			function reload(form)
			{
				...
				<?php
					...
					$query1 = "SELECT * FROM `m13` WHERE pnum='$pno' AND foreman='NPD' ";	
		...
		?>
				...
				<div class="column">
							...
							$t="";
								...
								else
								{
									...
									else
									{
										...
										/*$query = "SELECT *,(SUM(total_rec_qty)-IF(SUM(invqty) IS NULL,0,SUM(invqty))) AS stock FROM(SELECT dcnum,date,scn,pnum,SUM(total_rec_qty) AS total_rec_qty FROM `subcondb` WHERE pnum='$rat' AND scn LIKE '$rrat') AS TSUB
													LEFT JOIN(SELECT dcnum,pnum,SUM(invqty) AS invqty FROM `subcondb_invlink` WHERE pnum='$rat') AS TSUBINV ON TSUB.dcnum=TSUBINV.dcnum GROUP BY TSUB.pnum";
										*/
										...
										*/
										...
										//final query working correctly//good working but part from option is not in this query(Changes on 19-05-2021)
										//$query = "SELECT (rec-inv) AS stock FROM(SELECT SUM(total_rec_qty) AS rec,IF(SUM(invqty) IS NULL,0,SUM(invqty)) AS inv FROM(SELECT dcnum,date,scn,pnum,SUM(total_rec_qty) AS total_rec_qty FROM `subcondb` WHERE pnum='$rat' AND scn LIKE '$rrat' GROUP BY dcnum) AS TSUB LEFT JOIN(SELECT dcnum,pnum,IF(SUM(invqty) IS NULL,0,SUM(invqty)) AS invqty FROM `subcondb_invlink` WHERE pnum='$rat' GROUP BY dcnum ) AS TSUBINV ON TSUB.dcnum=TSUBINV.dcnum) AS FSTOCK";
										//if error occurs uncomment this query
										//above same query//$query = "SELECT (rec-inv) AS stock FROM(SELECT SUM(total_rec_qty) AS rec,IF(SUM(invqty) IS NULL,0,SUM(invqty)) AS inv FROM(SELECT dcnum,date,scn,pnum,SUM(total_rec_qty) AS total_rec_qty FROM `subcondb` WHERE pnum='$rat' AND scn LIKE '$rrat' GROUP BY dcnum) AS TSUB LEFT JOIN(SELECT dcnum,pnum,IF(SUM(invqty) IS NULL,0,SUM(invqty)) AS invqty FROM `subcondb_invlink` WHERE pnum='$rat' GROUP BY dcnum ) AS TSUBINV ON TSUB.dcnum=TSUBINV.dcnum) AS FSTOCK";
										
										//get pn part qty(parts from option)
										//$query = "SELECT (rec-inv) AS stock FROM(SELECT SUM(total_rec_qty) AS rec,IF(SUM(invqty) IS NULL,0,SUM(invqty)) AS inv FROM(SELECT dcnum,date,scn,pnum,SUM(total_rec_qty) AS total_rec_qty FROM `subcondb` WHERE pnum='$rat' OR pnum='$pnum' AND scn LIKE '$rrat' GROUP BY dcnum) AS TSUB LEFT JOIN(SELECT dcnum,pnum,IF(SUM(invqty) IS NULL,0,SUM(invqty)) AS invqty FROM `subcondb_invlink` WHERE pnum='$rat' OR pnum='$pnum' GROUP BY dcnum ) AS TSUBINV ON TSUB.dcnum=TSUBINV.dcnum) AS FSTOCK";
					...
					<div class="column">
						...
						<select name="igst_on_intra" id="igst_on_intra" required>
							<option value="">SELECT TYPE</option>
					...
					<div class="column">
						...
						<select name="reverse_charge" id="reverse_charge" required>
							<option value="">SELECT TYPE</option>
							<option value="Yes">Yes</option>
							<option value="No">No</option>
						</select>
