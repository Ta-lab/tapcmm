...
if(isset($_SESSION['user']))
{
	...
	if($_SESSION['user']=="123" || $_SESSION['user']=="133" || $_SESSION['access']=="ALL")
	{
		$id=$_SESSION['user'];
		$activity="DC RAISING";
...
<body style="background-image: url('img/6.jpg');">
	...
	<h4 style="text-align:center"><label> DC ENTRY VSS UNIT 2</label></h4>
	...
	<script>
		...
		function preload(form)
		{
			...
			self.location=<?php echo"'alfatestunit2.php?dc='"?>+p0+'&tdate='+p1+'&iss='+p;
		}
		function reload(form)
		{
			...
			self.location=<?php echo"'alfatestunit2.php?dc='"?>+p0+'&tdate='+p1+'&sccode='+p2+'&iss='+p;
		}
		function reload0(form)
		{
			...
			self.location=<?php echo"'alfatestunit2.php?tdate='"?>+p1+'&dc='+p0+'&sccode='+p2+'&partnumber='+p3+'&iss='+p;
	...
	<div class="divclass">
		<form action="alfatestunit2_db.php" method="post" enctype="multipart/form-data">
			<div id="stylized" class="myform"><br>
						...
						<input style="width: 60%; background-color:white;" type="text" id="p0" name="dc" readonly value="<?php
							...
							else
							{
								...
								*/
								...
								$q = "select dcnum as gen from unit2_dc_det ORDER BY dcnum DESC LIMIT 1";
								$r = $con->query($q);
								$fch=$r->fetch_assoc();
				...
				<datalist id="clist" >
					<?php
						if(isset($_GET['iss']))
						{
							...
							echo "<option selected value='".$sccode."'>".$sccode."</option>";
				...
				<script>
					function reload2(form)
					{
						...
						var p2 = document.getElementById("p2").value;
						var p3 = document.getElementById("p3").value;
						self.location=<?php echo"'alfatestunit2.php?tdate='"?>+p1+'&dc='+p0+'&sccode='+p2+'&partnumber='+p3+'&iss='+p+'&mat='+val;
					}
				</script>
					...
					if(isset($_GET['iss']) && $_GET['iss']=='FG For S/C' && isset($_GET['partnumber']) && $_GET['partnumber']!="" || $cnt>0)
					{
						...
						else
						{
							...
							if($c==1 || $cnt>0)
							{
								...
								//$result2 = $con->query("SELECT IF(dcqty-recqty IS NULL,0,dcqty-recqty) AS stock FROM (SELECT pnum,SUM(total_dc_qty) AS dcqty, SUM(total_rec_qty) AS recqty FROM `unit2receiveentry` WHERE pnum='$pnum') AS TU2 GROUP BY pnum ");
								$result2 = $con->query("SELECT SUM(total_rec_qty) AS stock FROM `unit2receiveentry` WHERE pnum='$rat' GROUP BY pnum");
								//echo "SELECT SUM(total_rec_qty) AS stock FROM `unit2receiveentry` WHERE pnum='$pnum' GROUP BY pnum";
								...
								$result3 = $con->query("SELECT SUM(qty) AS raised_dcqty FROM `unit2_dc_det` WHERE pn='$rat' GROUP BY pn");
								//echo "SELECT SUM(qty) AS raised_dcqty FROM `unit2_dc_det` WHERE pn='$rat' GROUP BY pn";
								$row3 = mysqli_fetch_array($result3);
								
								if($max==0 || $max<$row['stock'])
								{
									$max=$row['stock'] - $row3['raised_dcqty'];
								}
								}while($row = mysqli_fetch_array($result));
							}
							else
				...
				<?php
					if(isset($_GET['iss']) && $_GET['iss']=='FG For S/C' && isset($_GET['partnumber']) && $_GET['partnumber']!="" || $cnt>0)
					{
						if($max==1){$t="1000000";}else{ $extra=10000; $t=$max+$extra; }
				...
				<div class="column">
					<label>&nbsp;DC QTY&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
						<input type="number" style="width: 60%; background-color:white;" min="1" max="<?php if($max<1){ echo '" readonly="readonly';}else{echo $max+$extra;}?>" required id="p5" name="tiqty">
				</div>
